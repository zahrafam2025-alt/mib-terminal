...
(function () {
  ...
  function buildDesks(main) {
    ...
    if (dk.chart_read) {
      var SC = scene("chartread", "Chart Read", { wide: true });
      if (dk.chart_read.body) SC.inner.appendChild(rv(el("p", "desk-body", dk.chart_read.body), 1));
      var charts = dk.chart_read.charts || [];
      if (charts.length) {
        var grid = el("div", "chart-grid");
        charts.slice(0, 4).forEach(function (c) {
          var card = el("div", "chart-card");
          var head = el("div", "chart-head");
          head.appendChild(el("b", "", c.sym));
          if (c.price != null) head.appendChild(el("span", "px", Number(c.price).toLocaleString()));
          card.appendChild(head);
          if (c.svg) {
            var fig = el("div", "chart-fig");
            fig.innerHTML = c.svg;                 // trusted: built by our own build.py
            card.appendChild(fig);
          }
          if (c.caption) card.appendChild(el("p", "chart-cap", c.caption));
          grid.appendChild(card);
        });
        SC.inner.appendChild(rv(grid, 2));
      }
